...
function notifyIfChanged(businesses) {
  ...
  if (config.get("messageFilter.wordFilter")) {
    // save only word filtered businesses if filter is activated
    cache.businessesById = wordFilteredBusinesses(businessesById);
  } else {
...
function wordFilteredBusinesses(businessesById) {
  let wordFilteredBusinesses = {};
  Object.keys(businessesById)
    .filter((key) => {
      const storeName = businessesById[key].store.store_name;
      return isInWordFilter(storeName);
    })
    .forEach((key) => (wordFilteredBusinesses[key] = businessesById[key]));
  return wordFilteredBusinesses;
}
